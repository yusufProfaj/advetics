...
export class AiAssistantService {
  ...
  async confirm(ctx: TenantContext, conversationId: string, confirmationId: string): Promise<ToolResult> {
    ...
    let pending: { toolName: string; detail: PendingActionDetail } | null = null;
    let alreadyResolved = false;
    for (const row of rows) {
      const tr = row.toolResult as unknown as (ToolResult & { confirmationId?: string }) | null;
      if (!tr || tr.confirmationId !== confirmationId) continue;
      if (tr.status === 'pending_confirmation') {
        // ÇİFT CAST YOK — `detail` veritabanından gelen JSON, biçimi
        // GARANTİ DEĞİL. Denetimsiz bir `as unknown as X`, eksik/bozuk bir
        // alanı sessizce geçirirdi (CLAUDE.md: "eksik alan denetimini
        // tamamen kapatıyor").
        if (!isPendingActionDetail(tr.detail)) continue; // bozuk kayıt — "bulunamadı" ile aynı sonuç
        pending = { toolName: row.toolName!, detail: tr.detail };
      } else {
        // Aynı confirmationId ile 'in_progress' ya da TERMİNAL bir sonuç
        // zaten var. Bu YALNIZCA HIZLI YOL — sıradan bir ikinci tıklamayı
        // ucuza (platforma hiç gitmeden, hatta talep satırı yazmadan)
        // eleyip anlaşılır bir mesaj veriyor. GERÇEK kilit aşağıdaki
        // `claimConfirmation`: iki istek AYNI ANDA buraya geldiğinde ikisi de
        // satırları `pending` görür ve bu kontrol ikisini de geçirir.
