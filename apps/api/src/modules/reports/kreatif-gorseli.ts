...
/**
 * ÖZEL AĞ ARALIKLARI — logo indirmede DNS'ten dönen adres bunlara düşerse ret.
 *
 * Logo adresi ajansın KENDİ alan adında olduğu için beyaz liste
 * uygulanamıyor; korumayı adres yerine ÇÖZÜLEN IP üzerinden yapıyoruz.
 * `169.254.169.254` bulut metadata ucu, `127/8` ve `::1` yerel, `10/8`
 * `172.16/12` `192.168/16` iç ağ, `100.64/10` operatör NAT'ı.
 */
...
/**
 * LOGO İNDİRME — beyaz liste YOK, ama diğer bütün kontroller VAR.
 *
 * Kreatif görseli platformun CDN'inden geliyor ve orada bilinen sunucu
 * listesi işe yarıyor. Logo ise ajansın kendi alan adında; liste tutmak
 * her yeni müşteride kod değişikliği demek olurdu.
 *
 * Korumayı adres yerine ÇÖZÜLEN IP'ye taşıyoruz: DNS'ten dönen adres iç
 * ağa ya da bulut metadata ucuna düşüyorsa istek HİÇ yapılmıyor. Kalan
 * risk DNS rebinding; `redirect: 'manual'` ve kısa zaman aşımıyla birlikte
 * kabul edilen kalıntı risk bu — logoyu giren kişi zaten org yöneticisi.
 */
export async function logoIndir(
  ham: string,
  opts: { getir?: Getirici; cozumle?: (host: string) => Promise<string[]> } = {},
  ...
  const cozumle = opts.cozumle ?? varsayilanCozumle;
  let adresler: string[];
  try {
    adresler = await cozumle(url.hostname);
  } catch {
    return { ok: false, sebep: 'alan adı çözülemedi' };
  }
  if (adresler.length === 0) return { ok: false, sebep: 'alan adı çözülemedi' };
  if (adresler.some(ozelAdres)) return { ok: false, sebep: 'iç ağ adresi' };

  return indir(url, opts.getir);
}

async function varsayilanCozumle(host: string): Promise<string[]> {
  const { lookup } = await import('node:dns/promises');
  const kayitlar = await lookup(host, { all: true });
