...
export async function gorselIndir(
  ...
  const getir = opts.getir ?? fetch;
  const kendi = new AbortController();
  const zamanlayici = setTimeout(() => kendi.abort(), GORSEL_SINIRI.zamanAsimiMs);
  const disaridan = opts.signal;
  const iptal = (): void => kendi.abort();
  disaridan?.addEventListener('abort', iptal);

  try {
    const yanit = await getir(kontrol.url.toString(), {
      signal: kendi.signal,
    ...
    disaridan?.removeEventListener('abort', iptal);
  }
}
